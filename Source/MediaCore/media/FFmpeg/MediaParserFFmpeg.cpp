...
MediaParserFFmpeg::MediaParserFFmpeg(std::string url,
...
MediaParserFFmpeg::~MediaParserFFmpeg() {
  ...
  avformat_close_input(&_formatCtx);  // this function will free the _formatCtx
                                      // and _avioBuffer which malloc by youself
}
...
bool MediaParserFFmpeg::Init() {
  ...
  _avioCtx = avio_alloc_context(_avioBuffer, 1024, 0, this, readPacketCB, NULL,
                                seekMediaCB);
  if (!_avioCtx) {
  ...
  if ((_videoStreamId = av_find_best_stream(_formatCtx, AVMEDIA_TYPE_VIDEO, -1,
                                            -1, NULL, 0)) >= 0) {
  ...
  if ((_audioStreamId = av_find_best_stream(_formatCtx, AVMEDIA_TYPE_AUDIO, -1,
                                            -1, NULL, 0)) >= 0) {
  ...
  if (_videoStream) {
    ...
    _mediaInfo._videoDuration =
        (int64_t)(_formatCtx->duration) / 1000LL;  // millsecond
    ...
    if (avcodec_open2(_videoCodecCtx, _videoCodec, NULL) < 0) {
      ...
      printf("codec: %s, bit_rate:%d, wh:(%d,%d),  \n", _videoCodec->long_name,
             _videoCodecCtx->bit_rate, _videoCodecCtx->width,
  ...
  if (_audioStream) {
    ...
    printf(
        "sampleRate: <%d>, channles: <%d>, sampleFormat: <%s>, codec: <%s> , "
        "channleLayout: <%d> \n",
        _audioCodecCtx->sample_rate, _audioCodecCtx->channels,
        formatToString(_audioCodecCtx->sample_fmt), _audioCodec->long_name,
        (int)_audioCodecCtx->channel_layout);
  }  // if(_audioStream)
...
int MediaParserFFmpeg::readPacketCB(void* opaque,
...
int64_t MediaParserFFmpeg::seekMedia(int64_t offset, int whence) {
  if (whence == SEEK_SET) {
    ...
    _inputStream->seek(_inputStream->tell() +
...
int64_t MediaParserFFmpeg::seekMediaCB(void* opaque,
                                       int64_t offset,
                                       int whence) {
  MediaParserFFmpeg* tmpPtr = static_cast<MediaParserFFmpeg*>(opaque);
...
bool MediaParserFFmpeg::parseNextFrame() {
  ...
  if (!_formatCtx) {
    ...
    SetMediaParserState(PARSER_STATE_COMPLETE);
  }
  ...
  if (_seekFlag) {
    ...
    pkt->data = NULL;
    pkt->size = 0;
    // sync  av_read_frame and seek operation. when act the seek opeation,cann't
    // read
...
bool MediaParserFFmpeg::IsParseComplete() {
  return _mediaParserState == PARSER_STATE_COMPLETE;
}
...
uint64_t MediaParserFFmpeg::GetPacketDTS(AVPacket* packet,
...
uint64_t MediaParserFFmpeg::GetPacketBufferedLength() {
  ...
  if (!is_video_empty) {
    ...
    videoPacketBufferedLength =
        GetPacketDTS(end, video_time_base) - GetPacketDTS(beg, video_time_base);
  }

  if (!is_audio_empty) {
    beg = _audioPacketsQueue.front();
    end = _audioPacketsQueue.back();
    audioPacketBufferedLength =
        GetPacketDTS(end, audio_time_base) - GetPacketDTS(beg, audio_time_base);
  }

  if (!is_video_empty && !is_audio_empty) {
    buffer_length =
        std::min(videoPacketBufferedLength, audioPacketBufferedLength);
  } else {
    buffer_length =
        std::max(videoPacketBufferedLength, audioPacketBufferedLength);
  }
...
void MediaParserFFmpeg::clearBuffers() {
  if (isBufferEmpty()) {
    ...
    // clear videodeque;
    boost::mutex::scoped_lock(_mutexAudioQueue);
    boost::mutex::scoped_lock(__mutexVideoQueue);
...
bool MediaParserFFmpeg::Seek(int64_t millPos) {
  boost::mutex::scoped_lock lock1(_mutexAudioQueue);
  boost::mutex::scoped_lock lock2(_mutexVideoQueue);
  boost::mutex::scoped_lock readFrameLock(
      _readFrameMutex);  // sync with av_read_frame
  if (millPos < 0 || 0 /*millPos>_videoDuration*/) {
    return false;
  }
  if (av_seek_frame(_formatCtx, _videoStreamId,
                    (millPos / 1000) / videoTimeBase(), 0) < 0) {
    return false;
  }
